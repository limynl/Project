...
import com.limynl.project.utils.SizeUtils;
...
public class LineChartView extends View {
    ...
    private void reInitData() {
        ...
        lineSize = SizeUtils.dip2px(mContext, 1.5f);//折线粗细
        fillCircleRadio = SizeUtils.dip2px(mContext, 3.5f);//实心圆半径
        strokeCircleRadio = SizeUtils.dip2px(mContext, 4.25f);//空心圆半径
        fillPointSize = SizeUtils.dip2px(mContext, 1.5f);//实心圆粗细
        strokePointSize = SizeUtils.dip2px(mContext, 2f);//空心圆粗细
    }
    ...
    private void drewTable(Canvas canvas) {
        ...
        paint.setStyle(Style.STROKE);//FILL_AND_STROKE
        //画虚线的
        ...
        for (int i = 0; i < dates.size(); i++) {
            ...
            path.lineTo(marginLeft + i * widthInterval, marginTop + (values.size() - 1)* heightInterval);
            canvas.drawPath(path, paint);
        }
    }
    ...
    private void drawFoldLine(Canvas canvas) {
        ...
        paintFoldLine.setColor(Color.parseColor("#ff6900"));
        paintFoldLine.setStrokeWidth(lineSize);// 数据线宽度（粗细）
        paintFoldLine.setAntiAlias(true);
        paintFoldLine.setStyle(Style.STROKE);
    ...
    private void drawCircle(Canvas canvas) {
        ...
        fillPaint.setColor(Color.parseColor("#ff6900"));
        ...
        paintStroke.setColor(Color.parseColor("#ff6900"));
    ...
    private void drawRoundRect(Canvas canvas) {
        ...
        float skewingWidth = SizeUtils.dip2px(mContext, 5f);
        ...
        canvas.drawRoundRect(oval3, SizeUtils.dip2px(mContext, 7.7f), SizeUtils.dip2px(mContext, 5.5f), paint);// 第二个参数是x半径，第三个参数是y半径
    ...
    private void drawX(Canvas canvas) {
        ...
        paint.setStrokeWidth(1);
        paint.setAntiAlias(true);
        paint.setStyle(Style.FILL);// 数字字体的样式 （粗细/实体或空心什么的...）
    ...
    private void drawY(Canvas canvas) {
        ...
        for (int i = 0; i < numbers.size(); i++) {
            canvas.drawText(numbers.get(i), width, i * heightInterval + height, paint);
        }
